...
namespace rover_controller
{
    ...
    const Eigen::Vector3d& pos)
  {
    ...
    // avoid collision
    F += obstacleRepulsiveForce(obstacles, pos);

    // add vortex/curl field
    F += obstacleVortexForce(obstacles, pos);
    ...
    const Eigen::Vector3d& goal, const Eigen::Vector3d& pos)
  {
    ...
    Eigen::Vector3d e = goal - pos;
    if(e.norm() > 1.0) e = e.normalized();
    ...
    std::vector<Obstacle> obstacles, const Eigen::Vector3d& pos)
  {
    ...
    {
      v << (pos - obstacles[i].pos).head(2), 0;//#>>>>TODO: 2d distance of obstacle and rover (note z component is zero!)
    ...
    std::vector<Obstacle> obstacles, const Eigen::Vector3d& pos)
  {
    ...
    for(size_t i = 0; i < obstacles.size(); ++i)
    {
      ...
      { 
        ...
        R.topLeftCorner(2,2) = Eigen::Rotation2Dd(-M_PI/2).matrix();
      }
      else
      {
        //#>>>>TODO: set the upper left 2x2 corner of the rotation matrix R
        //#>>>>TODO: to create + pi/2 = + 90 degree rotation
        R.topLeftCorner(2,2) = Eigen::Rotation2Dd(M_PI/2.0).matrix();//#>>>>TODO: 
      }
  ...
  void Controller::obstacleCallback(const object_msgs::ObjectsConstPtr& msg)
  {
    ...
    for(size_t i = 0; i < msg->objects.size(); ++i)
    {
      obstacles_[i].pos << msg->objects[i].pose.position.x, 
                           ...
                           msg->objects[i].pose.position.z; //#>>>>TODO: Save the 3d position

...
bool PlaneSegmentation::initalize(ros::NodeHandle& nh)
{
  ...
  plane_cloud_pub_ = nh.advertise<sensor_msgs::PointCloud2>("/plane_segmentation/plane_pointcloud", 1);
  //#>>>>TODO: advertise the pointcloud for the remaining points (objects)
  objects_cloud_pub_ = nh.advertise<sensor_msgs::PointCloud2>("/plane_segmentation/object_pointcloud", 1);
...
void PlaneSegmentation::update(const ros::Time& time)
{
  ...
  if(is_cloud_updated_)
  {
    ...
    //#>>>>TODO: publish both pointclouds obtained by segmentCloud()
      sensor_msgs::PointCloud2 ros_plane_msg;
      pcl::toROSMsg(*plane_cloud_, ros_plane_msg);
      ros_plane_msg.header.stamp = time;
      plane_cloud_pub_.publish(ros_plane_msg);
  
      sensor_msgs::PointCloud2 ros_objects_msg;
      pcl::toROSMsg(*objects_cloud_, ros_objects_msg);
      ros_objects_msg.header.stamp = time;
      objects_cloud_pub_.publish(ros_objects_msg);
...
bool PlaneSegmentation::preProcessCloud(CloudPtr& input, CloudPtr& output)
{
  ...
  try {
      tfListener_.waitForTransform(base_frame_, input->header.frame_id, ros::Time(0), ros::Duration(1.0));
...
bool PlaneSegmentation::segmentCloud(CloudPtr& input, CloudPtr& plane_cloud, CloudPtr& objects_cloud)
{
  ...
  if (n.dot(Eigen::Vector3f(0, 0, 1)) < 0) {
    n = -n;
    d = -d;
  }
  ...
  pass.setInputCloud(transf_cloud);
  pass.setFilterFieldName("z");
  pass.setFilterLimits(0.01, 0.50);
  ...
  pcl::transformPointCloud(*filterd_cloud, *objects_cloud, T_plane_base);
  return true;
}
